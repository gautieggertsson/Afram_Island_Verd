...
%
% Þykkar línur: Hodrick-Prescott-leitni (lambda = 100), sem hreinsar
% burt sveiflur sem standa skemur en um áratug, einkum gengissveiflur;
% daufar línur: árleg gildi. Ísland 2024 er sýnt sem lóðrétt bil, 24
% til 31 prósent, með kúlum á báðum endum.
%
...
assert(abs(100*T.SE_hp(end) - 4.6) < 0.2, 'SE-HP-leitni 2024 stenst ekki.');
assert(abs(100*T.FI_hp(end) - 8.9) < 0.2, 'FI-HP-leitni 2024 stenst ekki.');
...
plot(ax, ar, 100*T.FI_hp, '-', 'Color', BLUE, 'LineWidth', 2.2);
plot(ax, ar, 100*T.SE_hp, '-', 'Color', ORANGE, 'LineWidth', 2.2);
plot(ax, ar, 100*T.AT_hp, '-', 'Color', GREEN, 'LineWidth', 2.2);
...
text(ax, 2024.6, 100*T.FI_hp(end), 'Finnland', 'Color', BLUE, ...
    'FontWeight', 'bold', 'FontSize', 9.5);
text(ax, 2024.6, 100*T.SE_hp(end), 'Svíþjóð', 'Color', ORANGE, ...
    'FontWeight', 'bold', 'FontSize', 9.5);
text(ax, 2024.6, 100*T.AT_hp(end), 'Austurríki', 'Color', GREEN, ...
    'FontWeight', 'bold', 'FontSize', 9.5);
