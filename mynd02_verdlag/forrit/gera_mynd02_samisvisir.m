...
% A01-útgáfan ber fyrirsagnir og örvartexta birtu myndarinnar í greininni
% (Mynd 5); E011-útgáfan heldur almennu fyrirsögnunum.
variants = struct( ...
    'pli',    {"pli_E011", "pli_A01"}, ...
    'tag',    {"E011", "A01"}, ...
    'maxv',   {190, 195}, ...
    'titlL',  {sprintf('Aðferð Áfram Íslands:\nVerðlagi deilt með tekjum'), ...
               sprintf('Verðlag Áfram Íslands:\nVerðlagsvísi deilt með tekjum')}, ...
    'subL',   {'Einkaneysla heimila (E011), útreikningur 2024', ...
               'Útreikningur 2024: Ísland „ódýrt“'}, ...
    'titlR',  {sprintf('Sami verðlagsvísir\nán tekjudeilingar'), ...
               sprintf('Hefðbundin framsetning:\nVerðlagsvísir heimila')}, ...
    'subR',   {'Einkaneysla heimila (E011), verðlag 2024', ...
               'Verðlag 2024'}, ...
    'orsnid', {'%d. sæti þegar deilt er með tekjum\n%d. sæti á verðlagskvarðanum', ...
               '%d. sæti á kvarða Áfram Íslands\n%d. sæti á venjulegum verðlagskvarða'}, ...
    'orstada', {'mid', 'efst'});
...
function snyrta(ax, titl, sub, INK, MUT)
    ...
    % Leturstærðir hækkaðar um ~1,5 pt 16.8.2026 fyrir læsileika í síma.
    ax.YTick = []; ax.Box = 'off'; ax.YAxis.Visible = 'off';
    ax.XAxis.FontSize = 8.5; ax.XAxis.Color = MUT; ax.TickDir = 'out';
    title(ax, titl, 'FontSize',11, 'FontWeight','bold', 'Color',INK, ...
        'Units','normalized', 'Position',[0 1.055 0], ...
        'HorizontalAlignment','left');
    text(ax, 0, 1.018, sub, 'Units','normalized', ...
        'FontSize',8.9, 'Color',MUT);
end
...
function icey = spjald(ax, data, color, titl, sub, maxv, eu, eulab, d, RED, INK, MUT)
    ...
    for i = 1:n
        ...
        text(ax, max(v,0)+maxv*0.012, y, isl(v,d), ...
            'FontSize',tern(ice,8.7,7.9), ...
            'Color',tern(ice,RED,MUT), ...
            'FontWeight',tern(ice,'bold','normal'), ...
            'BackgroundColor','w', 'Margin',0.4, 'Clipping','off');
    end
    text(ax, eu, n+0.9, eulab, 'FontSize',8.0, ...
        'Color',MUT, 'Clipping','off');
    xlim(ax, [0 maxv]); ylim(ax, [0.3 n+1.6]);
    snyrta(ax, titl, sub, INK, MUT);
end

function pila(fig, axL, axR, yL, yR, txt, RED, xstart, hlid, stada)
    % stada: 'mid' setur textann miðjaðan við miðju örvarinnar;
    % 'efst' setur hann vinstrijafnaðan rétt neðan við upphaf hennar,
    % eins og í birtu mynd greinarinnar.
    ...
    if strcmp(stada, 'efst')
        pos = [p1(1) + 0.024, p1(2) - 0.085];
        jofnun = 'left';
    else
    ...
    text(ov, pos(1), pos(2), txt, 'Color',RED, 'FontSize',9.6, ...
        'FontWeight','bold', 'HorizontalAlignment',jofnun, ...
